...
WORDLIST_FILENAME = 'Unit 3 - Structured Types\Words.txt'
...
def isWordGuessed(secretWord, lettersGuessed):
    ...
    for char in secretWord:
        if char in lettersGuessed: guessed = True
        else: guessed = False; break
...
def getGuessedWord(secretWord, lettersGuessed):
    ...
    for char in secretWord:
        if char in lettersGuessed: result += char
        else: result += '_ '
...
def getAvailableLetters(lettersGuessed):
    ...
    for char in alphabet:
        if not char in lettersGuessed: result += char
